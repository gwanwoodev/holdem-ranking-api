document.addEventListener("DOMContentLoaded", () => {
  ...
  let userDeleteButtons = Array.from(
    document.querySelectorAll(".userDeleteButton")
  );
  ...
  const searchUser = async (value) => {
    ...
    for (let i = 0; i < responseJson.data.length; i++) {
      insertNewUser(
        responseJson.data[i].idx,
        "검색된유저",
        responseJson.data[i].name,
        responseJson.data[i].age,
        responseJson.data[i].totalMoney,
        newTable
      );
    }
  };
  ...
  const insertNewUser = (idx, rank, name, age, totalMoney, newTable) => {
    ...
    updateButton.addEventListener("click", async function (evt) {
      let res = await fetch(`https://holdemranking.com/api/user/${this.value}`);
  ...
  userAddButton.addEventListener("click", async (evt) => {
    evt.preventDefault();
    let formData = new FormData();
    let userProfile = document.querySelector("#userForm input[name=profile]")
      .files[0];
    ...
    let userLocation = document.querySelector("#userForm input[name=location]")
      .value;

    let years = Array.from(
      document.querySelectorAll("#userForm .recordWrapper input[name=year]")
    );
    let rallys = Array.from(
      document.querySelectorAll("#userForm .recordWrapper input[name=rally]")
    );
    let records = Array.from(
      document.querySelectorAll("#userForm .recordWrapper input[name=record")
    );
    let moneys = Array.from(
      document.querySelectorAll("#userForm .recordWrapper input[name=money]")
    );
    let userRecords = [];
    ...
    let response = await fetch("https://holdemranking.com/api/user", {
      method: "POST",
      body: formData,
    });
  ...
  let userImportButtons = Array.from(
    document.querySelectorAll(".userImportButton")
  );
  ...
  const importUpdateForms = (idx, name, age, location, profile, records) => {
    ...
    let locationField = document.querySelector(
      "#userUpdateForm input[name=location]"
    );
    let imageField = document.querySelector(
      "#userUpdateForm .updateImageField"
    );
    let recordWrapper = document.querySelector(
      "#userUpdateForm .recordWrapper2"
    );
  ...
  userUpdateButton.addEventListener("click", async function () {
    if(!this.value) {
      alert("유저 수정 버튼을 눌러 불러와주세요.");
      return;
    }

    let formData = new FormData();
    let userProfile = document.querySelector(
      "#userUpdateForm input[name=profile]"
    ).files[0];
    let userName = document.querySelector("#userUpdateForm input[name=name]")
      .value;
    let userAge = document.querySelector("#userUpdateForm input[name=age]")
      .value;
    let userLocation = document.querySelector(
      "#userUpdateForm input[name=location]"
    ).value;
    let updateImageField = document.querySelector(
      "#userUpdateForm .updateImageField"
    ).src;
    let parseImageSrc = updateImageField.split("https://holdemranking.com/")[1];

    let years = Array.from(
      document.querySelectorAll(
        "#userUpdateForm .recordWrapper2 input[name=year]"
      )
    );
    let rallys = Array.from(
      document.querySelectorAll(
        "#userUpdateForm .recordWrapper2 input[name=rally]"
      )
    );
    let records = Array.from(
      document.querySelectorAll(
        "#userUpdateForm .recordWrapper2 input[name=record"
      )
    );
    let moneys = Array.from(
      document.querySelectorAll(
        "#userUpdateForm .recordWrapper2 input[name=money]"
      )
    );
    let userRecords = [];
    ...
    let response = await fetch("https://holdemranking.com/api/user?_method=PUT", {
      method: "POST",
      body: formData,
    });
  ...
  const recordDelButton = document.querySelector(".recordDelButton");
  const recordDelButton2 = document.querySelector(".recordDelButton2");

  recordDelButton.addEventListener("click", (evt) => {
    evt.preventDefault();
    let addForms = Array.from(document.querySelectorAll("#userForm .recordWrapper .form-group"));
    let addFormsCount = addForms.length;
    let minCount = 4;

    if(addFormsCount <= minCount) {
      alert("더이상 삭제하실 수 없습니다.");
      return;
    }

    addForms[addFormsCount-1].remove();
    addForms[addFormsCount-2].remove();
    addForms[addFormsCount-3].remove();
    addForms[addFormsCount-4].remove();
  });


  recordDelButton2.addEventListener("click", (evt) => {
    evt.preventDefault();
    let addForms = Array.from(document.querySelectorAll("#userUpdateForm .recordWrapper2 .form-group"));
    let addFormsCount = addForms.length;
    let minCount = 4;
