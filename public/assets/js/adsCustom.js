document.addEventListener("DOMContentLoaded", (evt) => {
  ...
  const searchAds = async (value) => {
    ...
    for (let i = 0; i < responseJson.data.length; i++) {
      insertNewAds(
        ...
        responseJson.data[i].addr,
        responseJson.data[i].content,
        newTable
      );
    }
  };
  ...
  const insertNewAds = (idx, location, name, addr, content, newTable) => {
    ...
    updateButton.addEventListener("click", async function () {
      let res = await fetch(`https://holdemranking.com/api/ads/ad/${this.value}`);
  ...
  let adsDeleteButtons = Array.from(
    document.querySelectorAll(".adsDeleteButton")
  );
  ...
  let adsImportButtons = Array.from(
    document.querySelectorAll(".adsImportButton")
  );
  ...
  const importUpdateForms = (idx, name, location, addr, content, profile) => {
    ...
    let locField = document.querySelector(
      "#adsUpdateForm select[name=location]"
    );
    let addrField = document.querySelector("#adsUpdateForm input[name=addr]");
    let contentField = document.querySelector(
      "#adsUpdateForm input[name=content]"
    );
  ...
  adsUpdateButton.addEventListener("click", async function () {
    let formData = new FormData();
    let adsProfile = document.querySelector(
      "#adsUpdateForm input[name=profile]"
    ).files[0];
    let imageField = document.querySelector("#adsUpdateForm .updateImageField")
      .src;
    let nameField = document.querySelector("#adsUpdateForm input[name=name]")
    ...
    let parseImageSrc = imageField.split("https://holdemranking.com/")[1];
    ...
    let response = await fetch(
      `https://holdemranking.com/api/ads/${locField}?_method=PUT`,
      {
        method: "POST",
        body: formData,
      }
  ...
  adsAddButton.addEventListener("click", async (evt) => {
    evt.preventDefault();
    let formData = new FormData();
    let adsProfile = document.querySelector("#adsForm input[name=profile]")
      .files[0];
    ...
    let adsContent = document.querySelector("#adsForm input[name=content]")
      .value;
    ...
    let response = await fetch(`https://holdemranking.com/api/ads/${adsLoc}`, {
      method: "POST",
      body: formData,
    });
